...
bool Match(const char *str, const char *test) {
  return 0 == fnmatch(test, str, 0);
}

void ValidateFilePath(std::string &filepath, const std::string &ext) {
...
namespace util {
  ...
  std::string OpenDrive::Load(const std::string &filename) {
    const std::string opendrive_folder = LIBOPENDRIVE_TEST_CONTENT_FOLDER "/OpenDrive/";
